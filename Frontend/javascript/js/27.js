...
// sort() : 배열을 정렬 | 원본 자체를 변경하는 메소드
	// arr = [5, 4, 2, 6, 8, 6, 11, 43, 33, 10];
	// let arr_sort = arr.sort((a,b) => a-b); //오름차순
	// let arr_sort = arr.sort((a,b) => b-a); // 내림차순

	// 자바스크립트는 값을 새로운 변수에 넣는 것이 아닌 주소를 넣기 때문에 아래처럼 진행해야함
	// 원본데이터가 아닌 별도의 새로운 배열을 생성 ( VALUE Copy 방식 )
...
// 모든 요소의 값 * 2의 결과를 배열로 획득
	// let arr_map = arr.map(num => num * 2);

// 3의 배수는 '짝'
	let arr_map = arr.map(num => {
		if( num % 3 === 0 ) {
			return '짝'
		} else {
			return num;
...
// some() : 판별 함수를 통과하는 요소가 있는지 판별해서 하나라도 있으면 true(return boolean)
