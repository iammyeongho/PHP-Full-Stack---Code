...
class TokenUtil {
    ...
    /**
     * 토큰에서 페이로드의 특정 키의 값을 리터
     * 
     * @param   string $token 토큰
     * @param   string $key 페이로드 키
     * @return  srting 페이로드 키에 해당하는 값
     */
    public function getPayloadValueTokey(string $token, string $key) {
        list($header, $payload, $signature) = $this->explodeToken($token);

        // STD 객체로 옴
        $payloadDecode = json_decode(EncrypUtil::base64UrlDecode($payload));

        if(is_null($payloadDecode) || !isset($payloadDecode->$key)) {
            throw new Exception('E05');
        }

        return $payloadDecode->$key;
    }
     ...
     * 리프래쉬 토큰 DB 저장
     * 
     * @param string $refreshToken 리프래쉬 토큰
     * @return boolean true
     */
